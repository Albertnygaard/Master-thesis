...
spatially. 
Used survey: ESS wave 2
*/

clear all
set more off

global path "C:\Users\Albert\Documents\Polit\Speciale_local\Data\ESS\ESS2"
...
clear all
use "$path\Pub_ETH_HouseholdGeovars_Y2.dta"

keep household_id household_id2 ea_id2 fsrad3_agpct fsrad3_lcmaj ///
lat_dd_mod lon_dd_mod sq7
...
clear all
use "$path\Pub_ETH_PlotGeovariables_Y2.dta"
...
keep household_id household_id2 avg_dist_household avg_plot_twi
...
** sect_cover_hh **
clear all
use "$path\sect_cover_hh_w2.dta"

keep pw2 household_id household_id2 rural pw saq01 saq02 saq03 saq06 saq07 hh_saq13_a hh_saq13_b ///
hh_saq13_c
...
/* conversion of time
in Ethiopian calendar, 1 May 2006 is 10 Jan 2014, and 23 May 2006 is 1 Feb 2014
in the Gregorian calendar. Link to converter.
https://www.funaba.org/cc */

gen year_greg = 2014
replace year_greg = 2015 if hh_saq13_a>22 & hh_saq13_b>3 & hh_saq13_c==2007
replace year_greg = 2013 if hh_saq13_a<23 & hh_saq13_b<5 & hh_saq13_c==2006
...
save "$path\Relevant data\sect_cover_hh_w2.dta", replace


** sect1_hh_w2 **
clear all
use "$path\sect1_hh_w2.dta" 

keep individual_id ea_id household_id household_id2 hh_s1q02 hh_s1q03 hh_s1q04_a
...
sa "$path\Relevant data\sect1_hh_w2.dta", replace

** sect1b_com_w2
clear all
use "$path\sect1b_com_w2.dta"
...
sa "$path\Relevant data\sect1b_com_w2.dta", replace

** sect2_hh_w2 **

clear all
use "$path\sect2_hh_w2.dta"
...
sa "$path\Relevant data\sect2_hh_w2.dta", replace

**  sect3_com_w2 **
clear all
use "$path\sect3_com_w2.dta"
...
sa "$path\Relevant data\sect3_com_w2.dta", replace

** sec3_pp_w2 **
clear all
use "$path\sect3_pp_w2.dta"
...
sa "$path\Relevant data\sect3_pp_w2.dta", replace
...
sa "$path\Relevant data\sect3_pp_w2_scraped.dta", replace

** sect4_hh_w2 **
clear all
use "$path\sect4_hh_w2.dta"
...
sa "$path\Relevant data\sect4_hh_w2.dta", replace

** sect4_com_w2 **
clear all
use "$path\sect4_com_w2.dta"
...
sa "$path\Relevant data\sect4_com_w2.dta", replace

** sect4_pp_w2 ** 
clear all
use "$path\sect4_pp_w2.dta"
...
sa "$path\Relevant data\sect4_pp_w2", replace

** sect5_pp_w2 ** 
clear all
use "$path\sect5_pp_w2.dta"
...
sa "$path\Relevant data\sect5_pp_w2", replace
...
keep holder_id household_id household_id2 ea_id ls_s8aq13b TLU_conv ls_s8aq00
gen TLU_hh = ls_s8aq13b * TLU_conv
bysort household_id2: egen TLU = total(TLU_hh)
...
keep household_id household_id2 ea_id avg_TLU
sa "$path\Relevant data\sect8a_ls_w2.dta", replace


** sect6_com_w2 **

clear all
use "$path\sect6_com_w2.dta"
...
sa "$path\Relevant data\sect6_com_w2", replace

** sect7_com_w2 **
clear all
use "$path\sect7_com_w2.dta"
...
sa "$path\Relevant data\sect7_com_w2", replace

** sect7_hh_w2 **
clear all
use "$path\sect7_hh_w2"
...
sa "$path\Relevant data\sect7_hh_w2.dta", replace

** sect9_com_w2 **
clear all
use "$path\sect9_com_w2.dta"
...
sa "$path\Relevant data\sect9_com_w2.dta", replace

** sect9_ph_w2 **
clear all
use "$path\sect9_ph_w2.dta"
...
merge 1:1 household_id2 field_id parcel_id using "$path\Relevant data\sect3_pp_w2.dta", nogen
keep household_id2 parcel_id field_id ph_s9q12 ph_s9q13 tot_land_area tot_hh_land
...
sa "$path\Relevant data\sect9_ph_w2.dta", replace

** sect13_hh_w2 **
clear all
use "$path\sect13_hh_w2.dta"
...
sa "$path\Relevant data\sect13_hh_w2.dta", replace

** sect8_hh_w2 ******
clear all
use "$path\sect8_hh_w2.dta"
...
sa "$path\Relevant data\sect8_hh_w2.dta", replace

** sect7_pp_w2 ******************************************************************************************
clear all 
use "$path\sect7_pp_w2.dta"
...
sa "$path\Relevant data\sect7_pp_w2.dta", replace



** cons_agg_w2 **
clear all
use "$path\cons_agg_w2.dta"
...
sa "$path\Relevant data\cons_agg_w2.dta", replace


*** merge ***
clear all
use "$path\Relevant data\sect_cover_hh_w2.dta"
...
merge m:1 ea_id2 using "$path\Relevant data\sect1b_com_w2.dta", nogen
merge m:1 ea_id2 using "$path\Relevant data\sect3_com_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect3_pp_w2_scraped.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect4_hh_w2.dta", nogen
merge m:1 ea_id2 using "$path\Relevant data\sect4_com_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect4_pp_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect5_pp_w2.dta", nogen
merge m:1 ea_id2 using "$path\Relevant data\sect6_com_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect7_hh_w2.dta", nogen
merge m:1 ea_id2 using "$path\Relevant data\sect9_com_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect9_ph_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect13_hh_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\cons_agg_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect2_hh_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect8_hh_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect7_pp_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect1_hh_w2.dta", nogen
merge 1:1 household_id2 using "$path\Relevant data\sect8a_ls_w2.dta", nogen

sa "$path\Relevant data\ESS2.dta", replace
**
